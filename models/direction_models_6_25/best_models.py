...
for model_dir in glob(os.path.join(ROOT_DIR, 'direction_1_*')):
    ...
    for trial_dir in glob(os.path.join(model_dir, 'trial_*')):
        ...
        val_auc = val_auc or 0
        val_loss = val_loss or 1e6

        # Composite score (you can tweak this formula)
        composite_score = val_accuracy + val_auc - (val_loss / 1000)
        row = {
    ...
    if trials_data:
        ...
        top5_df = df_model.sort_values(by='composite_score', ascending=False).head(5)
