...
def test_ict_taxonomy_snapshot_separates_setup_families_from_trade_types() -> None:
    ...
    assert "ict_classic_breaker" in snapshot["research_label_families"]
    assert snapshot["setup_type_to_label_family_overrides"]["classic_breaker"] == "ict_classic_breaker"
...
def test_ict_trade_type_inference_maps_setup_and_label_layers() -> None:
    ...
    assert infer_ict_trade_type_from_setup_type("classic_breaker") == "reversal"
    assert infer_ict_label_family_from_setup_type("classic_breaker") == ICT_CLASSIC_BREAKER_LABEL_FAMILY
