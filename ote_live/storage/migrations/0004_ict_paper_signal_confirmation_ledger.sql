CREATE TABLE IF NOT EXISTS ict_paper_signal_events (
    ...
    holding_period_bars INTEGER NOT NULL CHECK (holding_period_bars = 20),
    exit_timing TEXT NOT NULL CHECK (exit_timing = 'close_after_20_completed_bars'),
    stop_price REAL,
    target_price REAL,
...
CREATE INDEX IF NOT EXISTS idx_ict_paper_signal_events_pending
    ON ict_paper_signal_events (lifecycle_status, asset, timeframe, entry_timestamp_utc);

CREATE INDEX IF NOT EXISTS idx_ict_paper_signal_events_source
    ON ict_paper_signal_events (bundle_id, model_id, source_timestamp_utc, source_row_idx);
