...
CODE_ARCHIVE="${CODE_ARCHIVE:-$HOME/trade_bot_code.tgz}"
PREPARED_ARCHIVE="${PREPARED_ARCHIVE:-$HOME/prepared_target.tgz}"
WORKDIR="${WORKDIR:-$HOME/trade_bot}"
...
OUTPUT_ROOT_REL="${OUTPUT_ROOT_REL:-models/deepinfra_1min_tcn_${TARGET}_${RUN_ID}}"
...
print_runtime_summary() {
  ...
  echo "=================================================="
  echo "DeepInfra 1-Minute TCN Full Run"
  echo "Target: $TARGET"
...
run_training() {
  ...
  mkdir -p logs
  local log_file="logs/deepinfra_tcn_${TARGET}_${RUN_ID}.log"
