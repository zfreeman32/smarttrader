...
class ClassificationConfigManager:
    ...
    """
    def __init__(self):
        # Data settings - Updated for your dataset
        self.data_file = 'long_EURUSD_1min_filtered.csv'  # Your input file
        self.results_file = "buy_class_model_training_results.txt"
        
        # Model parameters
        self.target_col = 'long_signal'
        self.lookback_window = 60  # Reduced for faster training with your features
        self.test_size = 0.2
        self.random_seed = 42
        
        # Memory optimization settings - Adjusted for your dataset size
        self.max_samples = 1000000   # Adjust based on your dataset size and memory
        self.chunk_size = 20000      # Smaller chunks for stability
        self.use_streaming = True   # Use tf.data for memory efficiency
        
        # Training parameters
        self.batch_size = 512       # Balanced for your feature count
        self.max_epochs = 30
        self.steps_per_epoch = 100  # Increased for better training
        self.validation_steps = 50  # Increased for better validation
        self.early_stopping_patience = 7
        self.initial_learning_rate = 1e-4
        
        # Model selection - Focus on models that work well with your features
        self.selected_models = ['Conv1D_LSTM']
        
        # Data transformation
        self.use_lag_features = True
        self.lag_periods = [5, 10, 20, 30, 70, 95, 65]  # Shorter lags for more recent patterns
        
        # Hyperparameter tuning
        self.max_trials = 150        # Balanced for thorough search
        self.max_consecutive_failed_trials = 40
        
        # Feature selection settings
        self.use_important_features_only = True  # New flag to use only important features
        self.exclude_future_looking = True       # New flag to exclude future-looking features
...
def load_and_preprocess_data(config):
    ...
    # Handle duplicate columns (especially Volume which appears twice)
    ...
    # Option 1: Use only the important features you identified
    important_features = [
        'Volume', 'RSI', 'CCI', 'WILLR', 'MACD', 'MACDHIST', 'STOCHRSI_D', 'STOCHRSI_K',
        'PLUS_DI', 'MINUS_DI', 'MINUS_DM', 'ROC', 'ROCP', 'CMO', 'BOP', 'TRIX', 'MFI',
        'ULTOSC', 'ADX', 'ADXR', 'DX', 'TRANGE', 'rolling_std', 'z_score', 'VWAP', 'EFI',
        'AD', 'ADOSC', 'OBV', 'HT_DCPHASE', 'HT_DCPERIOD', 'HT_SINE', 'HT_LEADSINE',
        'AROON_DOWN', 'AROON_UP', 'STOCH_D', 'STOCHF_K', 'APO', 'current_candle_height',
        'average_candle_height', 'williams_buy_signal', 'acc_dist_sell_signal',
        'rsi_oversold_signal', 'cci_bullish_signal', 'volatility_band_buy_signal',
        'kc_sell_signal', 'stochrsi_oversold_signal', 'ironbot_sell_signal', 'PPO',
        'golden_cross_buy_signal'
    ]
    ...
    # Alternative Option 2: Use all features except excluded ones (uncomment if preferred)
    # feature_columns = [col for col in data.columns if col not in exclude_columns]
    # print(f"Using {len(feature_columns)} features from the dataset")
    ...
    if config.use_lag_features:
        ...
        for lag in lag_list:
            ...
            for indicator in ['RSI', 'CCI', 'WILLR', 'MACD', 'ROC']:
