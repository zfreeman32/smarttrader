...
def build_Dense_layer(hp):
    ...
    return Dense(
        ...
        activation=hp.Choice("activation", ['tanh', 'relu', 'sigmoid', 'softplus', 'elu', 'linear', 'gelu']),
        kernel_initializer=hp.Choice("kernel_initializer", ['glorot_normal','he_normal']),
        bias_initializer=hp.Choice("bias_initializer", ['zeros', 'glorot_normal','he_normal']),
...
def build_SimpleRNN_layer(hp, return_sequences=False):
    ...
    return SimpleRNN(
        ...
        activation=hp.Choice("activation", [ 'relu', 'tanh', 'softplus', 'elu', 'gelu' ]),
        use_bias=True,
        kernel_initializer=hp.Choice("kernel_initializer", ['glorot_normal','he_normal']),
        ...
        recurrent_dropout=hp.Float("recurrent_dropout", min_value=0.1, max_value=0.4, step=0.05),
...
def build_LSTM_layer(hp, return_sequences=False):
    ...
    return LSTM(
        units=hp.Int("units_first", min_value=32, max_value=128, step=32),
        activation=hp.Choice("activation", [ 'relu', 'tanh', 'gelu' ]),
...
def build_Conv1D_layer(hp, data_format='channels_last'):
    return Conv1D(
        ...
        bias_initializer='zeros',  # Fixed
        kernel_regularizer=tf.keras.regularizers.l2(1e-5)  # Light regularization
    )
...
def build_ConvLSTM1D_layer(hp, return_sequences=False, data_format='channels_last'):
    '''Use only if you specifically need ConvLSTM - very resource intensive'''
    return ConvLSTM1D(
        ...
        recurrent_activation='sigmoid',  # Fixed
        kernel_initializer='glorot_normal',  # Fixed
        recurrent_initializer='orthogonal',  # Fixed
        dropout=0.2,  # Fixed
        recurrent_dropout=0.2,  # Fixed
        return_sequences=return_sequences
    )
