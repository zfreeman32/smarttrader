...
class AdvancedFeatureSelector:
    ...
    def method_3_regularization_based(self, X, y):
        """Method 3: Improved Regularization for direction prediction"""
        print("Method 3: Regularization-based Selection (Direction Optimized)")
        ...
        if self.regression_mode:
            # More lenient for regression
            alphas = np.logspace(-6, 1, 50)
            selector = LassoCV(alphas=alphas, cv=3, random_state=self.random_state, 
                            n_jobs=-1, max_iter=3000)
        else:
            # ElasticNet for classification (less aggressive than pure L1)
            selector = LogisticRegressionCV(
                Cs=np.logspace(-3, 3, 20), cv=3, penalty='elasticnet',
                solver='saga', l1_ratios=[0.1, 0.3, 0.5, 0.7],
                random_state=self.random_state, n_jobs=-1, max_iter=3000
            )
        ...
        if hasattr(selector, 'coef_'):
            coefficients = np.abs(selector.coef_[0] if not self.regression_mode else selector.coef_)
        else:
        ...
        non_zero_coefs = coefficients[coefficients > 1e-10]
        if len(non_zero_coefs) > 0:
            threshold = max(1e-10, np.percentile(non_zero_coefs, 5))  # Top 95%
        else:
            threshold = 1e-12
        
        selected_features = [feat for feat, coef in feature_scores.items() if coef > threshold]
        
        # Ensure minimum features
        if len(selected_features) < max(10, self.target_features // 4):
            selected_features = sorted(X.columns, key=lambda x: feature_scores[x], 
                                    reverse=True)[:self.target_features]
        else:
            selected_features = sorted(selected_features, key=lambda x: feature_scores[x], 
                                    reverse=True)[:self.target_features]
        ...
        print(f"Selected {len(selected_features)} features with improved regularization")
        return selected_features
...
def create_profit_aware_targets(df, signal_col, profit_window=14, stop_loss_pct=0.01):
    ...
    if signal_col not in df.columns:
        log(f"Warning: Signal column '{signal_col}' not found in dataframe", LOG_LEVEL_WARNING)
        return pd.Series(0, index=df.index, name='profitable_signal')
    
    required_price_cols = ['Close', 'High', 'Low']
    missing_cols = [col for col in required_price_cols if col not in df.columns]
    if missing_cols:
        log(f"Warning: Required price columns missing: {missing_cols}", LOG_LEVEL_WARNING)
        return pd.Series(0, index=df.index, name='profitable_signal')
    
    # Create a copy to avoid modifying the original dataframe
    df_work = df.copy()
    temp_cols = []  # Track columns we create
    
    try:
        if signal_col == 'long_signal':
            # For long signals, check if price increased
            df_work['entry_price'] = df_work['Close'].shift(-1)  # Entry at next bar's close
            df_work['exit_price'] = df_work['Close'].shift(-profit_window)  # Exit after profit_window
            temp_cols.extend(['entry_price', 'exit_price'])
            
            # Calculate stop loss price
            df_work['stop_price'] = df_work['entry_price'] * (1 - stop_loss_pct)
            temp_cols.append('stop_price')
            
            # Check if stop was hit before profit target
            hit_stop = pd.Series(False, index=df_work.index)
            for i in range(1, profit_window):
                # If any Low price between entry and exit is below stop price
                col_name = f'hit_stop_{i}'
                df_work[col_name] = df_work['Low'].shift(-i) < df_work['stop_price']
                temp_cols.append(col_name)
                hit_stop = hit_stop | df_work[col_name]
                
            # Calculate PnL (exit_price - entry_price)
            df_work['pnl'] = df_work['exit_price'] - df_work['entry_price']
            temp_cols.append('pnl')
            
            # Signal is profitable if PnL > 0 and stop loss wasn't hit
            profitable = ((df_work['pnl'] > 0) & ~hit_stop & (df_work[signal_col] == 1)).astype(int)
            ...
            for i in range(1, profit_window):
                ...
                col_name = f'hit_stop_{i}'
                df_work[col_name] = df_work['High'].shift(-i) > df_work['stop_price']
                temp_cols.append(col_name)
                hit_stop = hit_stop | df_work[col_name]
                
            # Calculate PnL (entry_price - exit_price) for shorts
            df_work['pnl'] = df_work['entry_price'] - df_work['exit_price']
            temp_cols.append('pnl')
            ...
            # For other signal types, return a default (all zeros or copy of original)
            log(f"Warning: Unsupported signal column '{signal_col}' for profit analysis", LOG_LEVEL_WARNING)
            log(f"Returning copy of original signal column")
            if df_work[signal_col].dtype in ['int64', 'int32', 'bool']:
                profitable = df_work[signal_col].astype(int)
            else:
                profitable = pd.Series(0, index=df_work.index)
        
        # Clean up temporary columns (only drop columns that actually exist)
        existing_temp_cols = [col for col in temp_cols if col in df_work.columns]
        if existing_temp_cols:
            df_work.drop(columns=existing_temp_cols, inplace=True)
        
        log(f"Profit-aware targets created for {signal_col}: {profitable.sum()} profitable signals out of {(df[signal_col] == 1).sum()} total signals")
        
        return profitable
        
    except Exception as e:
        log(f"Error creating profit-aware targets for {signal_col}: {e}", LOG_LEVEL_ERROR)
...
class EnhancedTradingDataAnalyzer:
    def __init__(self, df, target_col='long_signal', regression_mode=False, forecast_periods=14):
        self.df = df.copy()
        ...
        # Prepare targets based on mode and target type
        if regression_mode:
            # For regression, create both absolute Close and directional targets
            if target_col.startswith('returns_'):
                # Target is already a returns column, use it directly
                self.target_col = target_col
                log(f"Using existing returns column: {target_col}")
            else:
                ...
                log(f"Created regression targets for forecast_periods={forecast_periods}")
            
            # Remove NaN rows from the target
            valid_rows_before = len(self.df)
            self.df = self.df.dropna(subset=[self.target_col])
            valid_rows_after = len(self.df)
            log(f"Regression mode: {valid_rows_before} -> {valid_rows_after} rows after removing NaN targets")
            
        else:
            # For classification, handle different target types
            if target_col.startswith('direction_'):
                # Target is already a direction column, use it directly
                self.target_col = target_col
                log(f"Using existing direction column: {target_col}")
                
            elif target_col in ['long_signal', 'short_signal']:
                # Create profit-aware targets for signal columns
                log(f"Creating profit-aware targets for signal column: {target_col}")
                ...
                # For other classification targets, use them directly
                self.target_col = target_col
                log(f"Using existing classification column: {target_col}")
            
            # Remove NaN rows from the target
            valid_rows_before = len(self.df)
            self.df = self.df.dropna(subset=[self.target_col])
            valid_rows_after = len(self.df)
            log(f"Classification mode: {valid_rows_before} -> {valid_rows_after} rows after removing NaN targets")
        
        # Validate target column exists and has valid data
        if self.target_col not in self.df.columns:
            raise ValueError(f"Target column '{self.target_col}' not found in dataframe")
        
        target_valid_count = self.df[self.target_col].notna().sum()
        if target_valid_count == 0:
            raise ValueError(f"Target column '{self.target_col}' has no valid data")
        
        log(f"Target column '{self.target_col}' has {target_valid_count} valid values")
        
        # Apply feature filtering
        log("Starting feature filtering...")
        self.feature_filtering_results = self._filter_features()
        log(f"Feature filtering complete: {self.feature_filtering_results['final_count']} features selected")
...
if __name__ == "__main__":
    ...
    # Configuration
    run_enhanced_selection = True
    target_features = 75
    run_shap = True
    max_shap_time = 30  # Reduced from 2048 to 30 minutes for practical use
    
    start_section("main")
    
    log(f"=== DIRECTION-BASED TRADING ANALYSIS ===")
    ...
    log("Loading and preprocessing data with direction targets")
    try:
        df = pd.read_csv(file_path)
        original_rows = len(df)
        log(f"Dataset loaded successfully: {df.shape}")
        
        # Check if required columns exist
        required_cols = ['Close']
        missing_cols = [col for col in required_cols if col not in df.columns]
        if missing_cols:
            log(f"ERROR: Required columns missing: {missing_cols}")
            exit(1)
        
        # Create direction targets with the fixed function
        df = create_direction_targets(df, periods=[1, 3, 5, 10, 14])
        
        # Save preprocessed data
        preprocessed_path = file_path.replace('.csv', '_with_directions.csv')
        df.to_csv(preprocessed_path, index=False)
        log(f"Preprocessed data: {original_rows} -> {len(df)} rows")
        log(f"Saved to: {preprocessed_path}")
    ...
    # STEP 3B: Updated configurations for direction prediction
    configurations = [
        # Keep original signals if they exist
        # {"target_col": "long_signal", "regression_mode": False},
        # {"target_col": "short_signal", "regression_mode": False},
        
        # NEW: Direction targets (much better for technical analysis)
        {"target_col": "direction_1", "regression_mode": False},      # Next bar
        {"target_col": "direction_5", "regression_mode": False},      # 5-bar ahead
        {"target_col": "direction_14", "regression_mode": False},     # 14-bar ahead
        
        # Multi-class with neutral zone
        {"target_col": "direction_3class_5", "regression_mode": False},
        
        # Regression alternatives
        {"target_col": "returns_1", "regression_mode": True},
        {"target_col": "returns_5", "regression_mode": True}
    ]
    ...
    for config in configurations:
        if config["target_col"] in df.columns:
            ...
            log(f"✅ Target '{config['target_col']}' available")
        else:
            log(f"⚠️  Target '{config['target_col']}' not found, skipping")
    
    configurations = valid_configurations
    
    if len(configurations) == 0:
        log("ERROR: No valid target columns found!")
        log(f"Available columns: {list(df.columns)}")
        exit(1)
    
    log(f"Running {len(configurations)} configurations")
    
    # STEP 3C: Print target statistics
    log("\n=== TARGET STATISTICS ===")
    for config in configurations:
        ...
        try:
            if config["regression_mode"]:
                ...
                log(f"{target_col}: mean={stats['mean']:.6f}, std={stats['std']:.6f}, non-null={stats['count']}")
            else:
                counts = df[target_col].value_counts().sort_index()
                pcts = (counts / len(df) * 100).round(1)
                log(f"{target_col}: {dict(zip(counts.index, pcts))}%")
        except Exception as e:
            log(f"Error getting stats for {target_col}: {e}")
    
    # STEP 3D: Execution mode
    cores_available = os.cpu_count()
    if GPU_AVAILABLE:
        log("🔥 GPU mode - Sequential execution")
        run_parallel = False
    elif cores_available >= 6:
        log("💻 CPU mode - Parallel execution")
        run_parallel = True
    else:
        log("🐌 Limited cores - Sequential execution")
        run_parallel = False
    
    # STEP 3E: Run configurations
    if run_parallel and len(configurations) > 1:
        log("Running configurations in parallel...")
        with ProcessPoolExecutor(max_workers=min(len(configurations), cores_available // 2)) as executor:
            futures = [executor.submit(run_configuration, config, preprocessed_path, 
                                     run_shap, max_shap_time, run_enhanced_selection, target_features) 
                      for config in configurations]
            ...
            for i, future in enumerate(futures):
                ...
                try:
                    ...
                    if result:
                        success_count += 1
                        log(f"✅ {config['target_col']}: SUCCESS")
                    else:
                        log(f"❌ {config['target_col']}: FAILED")
                except Exception as e:
                    log(f"❌ {config['target_col']}: ERROR - {e}")
            
            log(f"Parallel execution completed: {success_count}/{len(configurations)} successful")
    else:
        # Sequential execution
        log("Running configurations sequentially...")
        success_count = 0
        for i, config in enumerate(configurations, 1):
            log(f"🚀 {i}/{len(configurations)}: {config['target_col']}")
            try:
                success = run_configuration(config, preprocessed_path, run_shap, 
                                          max_shap_time, run_enhanced_selection, target_features)
                if success:
                    success_count += 1
                    log(f"✅ {config['target_col']}: SUCCESS")
                else:
        ...
        log(f"Sequential execution completed: {success_count}/{len(configurations)} successful")
    ...
    direction_cols = [col for col in df.columns if col.startswith('direction_')]
    returns_cols = [col for col in df.columns if col.startswith('returns_')]
    log(f"Direction targets created: {direction_cols}")
    log(f"Returns targets created: {returns_cols}")
    log(f"Configurations run: {len(configurations)}")
    log(f"Successful analyses: {success_count}")
    log("Files generated for each target:")
    log("  - {target}_analysis_results.txt")
    log("  - {target}_important_features.txt") 
    log("  - {target}_enhanced_selection_details.txt")
    log("  - models_{target}_*/ (saved models)")
    
    if success_count > 0:
        log("\n✅ Analysis completed successfully!")
        log("Check the generated files for results and selected features.")
    else:
