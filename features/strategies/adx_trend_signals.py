...
def adx_trend_signals(stock_df, length=14, lag=14, average_length=14, trend_level=25, max_level=50, crit_level=20, mult=2, average_type='simple'):
    ...
    signals['adx_trend_buy_signal'] = np.where(((signals['ADX'] > signals['min_ADX'] * mult) & (signals['ADX'] > crit_level) | (signals['ADX'] > trend_level) & (signals['ADX'] < max_level)) & (close > signals['avg_price']), 1, 0)
    signals['adx_trend_sell_signal'] = np.where(((signals['ADX'] > signals['min_ADX'] * mult) & (signals['ADX'] > crit_level) | (signals['ADX'] > trend_level) & (signals['ADX'] < max_level)) & (close < signals['avg_price']), 1, 0)
